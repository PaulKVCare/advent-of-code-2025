...
public class AoCDayRunner
{
    ...
    public Task Run(int day, bool writerEnabled = false)
    {
        if (!TryCreateSolutionInstance(day, out var solution))
        {
        ...
        RunInternal(solution, writerEnabled);
        return Task.CompletedTask;
    }

    private static void RunInternal(DayBase solution, bool writerEnabled)
    {
        solution.WriterEnabled = writerEnabled;
        
        for (var i = 0; i < 2; i++)
        {
            RunPartInternal(solution, part: i + 1);
        }
    }

    private static void RunPartInternal(DayBase solutionInstance, int part)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[Elapsed: {stopwatch.Elapsed}] Part {part} solution => {result}");
            Console.ResetColor();
        }
        catch (Exception e)
        {
    ...
    private static bool TryCreateSolutionInstance(int day, [NotNullWhen(true)] out DayBase? instance)
    {
        try
        {
            ...
            instance = (DayBase)Activator.CreateInstance(type)!;
            return true;
        }
        catch (Exception e)
        {
